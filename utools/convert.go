...
// ConvertSolarToLunar 公历转农历
func ConvertSolarToLunar(solarYear, solarMonth, solarDay int) [4]int {
	var date = fmt.Sprintf("%04d-%02d-%02d", solarYear, solarMonth, solarDay)
	// 1900-1-31 时间戳
	...
	hex := lunarInformation[lunarYear-1900]
	sumMonth := 0
	leapMonth := hex & 0xf
	isLeap := false
...
func ConvertLunarToSolar(year, month, day int, leap bool) [3]int {
	...
	// 加上最后一年
	hex := lunarInformation[year-1900]
	leapMonth := hex & 0xf
	lunarMonth := 1
	for i := 0x08000; i >= 0x00010; i >>= 1 {
		if month == lunarMonth {
...
func GetLunarLeapMonth(year int) int {
	return lunarInformation[year-1900] & 0xf
}
